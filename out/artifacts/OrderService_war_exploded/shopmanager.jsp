...
<script>
    ...
    function actionFormatter(value, row, index) {
        return [
            ...
            '<button class="btn btn-primary drink">进入</button>'
        ].join('');
    }
    ...
    window.actionEvents = {
        ...
        'click .delete': function (e, value, row, index) {
            $.ajax({
                ...
                success: function(data) {
                    if(data.success==1){
                        alert("删除成功");
                        var newpage="shopManager.do";
                        window.open(newpage)
                    }
                    else{
                        alert("删除失败");
                        var newpage="editShop.do?shop_id="+shop_id;
                        window.open(newpage);
                    }
        ...
        'click .drink': function (e, value, row, index) {
            var newpage="drinkManager.do?shop_id="+row.shop_id;
            window.open(newpage);
